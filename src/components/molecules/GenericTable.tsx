            ...
            {loading ? (
              Array.from({ length: 5 }).map((_, i) => (
                <TableRow key={`skeleton-${String(i)}`} className="border-b border-slate-100 last:border-0">
                  {dataOfColumn.map((col, index) => (
                    <TableCell key={`skeleton-${String(i)}-${String(index)}`} className="py-4 px-6">
              ...
              data.map((row, rowIndex) => (
                <TableRow
                  ...
                  key={String((row as Record<string, unknown>).id ?? (row as Record<string, unknown>).user_id ?? String(rowIndex))}
                >
                  {dataOfColumn.map((col) => {
                    const cellValue = (row as Record<string, unknown>)[col.key];
                    return (
                      <TableCell className="py-4 px-6 text-sm text-slate-700 font-medium" key={`${String((row as Record<string, unknown>).id ?? String(rowIndex))}-${col.key}`}>
                        {columnRenderers?.[col.key]
                          ? columnRenderers[col.key](String(cellValue ?? ""), row)
